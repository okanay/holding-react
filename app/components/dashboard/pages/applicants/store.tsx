...
export function DashboardApplicantProvider({ children }: PropsWithChildren) {
  ...
  return (
    <DashboardApplicantContext.Provider value={store}>
      {children}
    </DashboardApplicantContext.Provider>
  );
}

export function useDashboardApplicant() {
  const context = useContext(DashboardApplicantContext);

  if (!context) {
    throw new Error(
      "useDashboardApplicant hook must be used within a DashboardApplicantProvider",
    );
  }
...
const DashboardApplicantContext =
  createContext<DashboardApplicantContextType>(undefined);
type DashboardApplicantContextType = StoreApi<DataState> | undefined;
